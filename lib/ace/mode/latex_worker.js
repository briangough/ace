...
(function() {
    ...
    this.getContext = function(row, column) {
      return this.document.getContext(row, column);
    }
...
var DocumentTree = function(TokeniseResult) {
    ...
    this.getPreviousDelimiter = function() {
        var node = this.getNthPreviousNode(0);
        if (!node)
            return null
    ...
    this.getContext = function(row, column) {
        var index = TokeniseResult.linePosition[row] + column;
        var context = [];
        var node = tree;
        while (node != null) {
            var matched = false;
            for (var i = 0; i < node.children.length; i++) {
                ...
                if (child.startDelimiter && child.endDelimiter) {
                    var startIndex = child.startDelimiter.token[2];
                    // "\begin{equation}" -> "\begin", "{", "equation", "}" as tokens.
                    // We want the index of the last token in the delimiter
                    var endToken = child.endDelimiter.closeToken || child.endDelimiter.token;
                    var endIndex = endToken[3];
                    if (index >= startIndex && index <= endIndex) {
                        ...
                        if (child.startDelimiter.mathMode === null) {
                            contextView.mathMode = false
                        } else if (typeof child.startDelimiter.mathMode === "object") {
                            contextView.mathMode = true
                        }
                        if (node.startDelimiter.command == "begin") {
                            contextView.environment = node.startDelimiter.name
                        } else {
                            contextView.delimiter = node.startDelimiter.command
                        }
                        context.push(contextView);
                        matched = true;
                        break;
                    }
                }
        ...
        do {
            ...
            if (closedBy(lastDelimiter, thisDelimiter)) {
                ...
                if (delimiterPrecedence(lastDelimiter) < delimiterPrecedence(thisDelimiter)) {
                    // discard the lastDelimiter by not pushing it back on the stack
                    // then retry the match for thisDelimiter
...
var Parse = function (text) {
    ...
    return {
      ...
      document: Environments.getDocument()
    }
};
