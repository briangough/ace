...
var DocumentTree = function() {
    ...
    var stack = [tree]; // TODO: Don't expose this
    
    this.open = function(env) {
        var current_node = this.getCurrentNode();
        var new_node = {
            env: env,
            children: []
        };
    ...
    this.getCurrentEnv = function() {
        return this.getCurrentNode().env;
    };
    
    this.getPreviousEnv = function() {
        if (stack.length <= 2) {
            // Can't return env of root node.
            return null;
        }
        return stack[stack.length - 2].env;
    }
...
var EnvHandler = function (ErrorReporter) {
    ...
    var document = this.document = new DocumentTree();
    ...
    this.push = function (newEnv) {
        ...
        //var p = document.length ;
        //var mathModeBefore = document.length > 0 ? !!state[document.length-1].mathMode : null;
        this.checkAndUpdateState(newEnv);
        envs.push(newEnv);
        // Debugging code for math mode transitions
        // var q = document.length;
        // if (true || newEnv.name) {
        //     var mathModeAfter = document.length > 0 ? !!state[document.length-1].mathMode : null;
        //     var cmd = newEnv.command;
        //     if (newEnv.name) { cmd += "{" + newEnv.name + "}";};
        //     console.log(cmd, mathModeBefore, mathModeAfter);
        // }
    };

    this._endVerbatim = function (thisEnv) {
        var lastEnv = document.getCurrentEnv();
        ...
        do {
            ...
            if (closedBy(lastEnv, thisEnv)) {
                ...
                if (envPrecedence(lastEnv) < envPrecedence(thisEnv)) {
                    // discard the lastEnv by not pushing it back on the stack
                    // then retry the match for thisEnv
                    ...
                    var prevLastEnv = document.getPreviousEnv();
                    if(prevLastEnv) {
                        if (thisEnv.name === prevLastEnv.name) {
                            //  got a match on the previous environment
                            document.close() // Close current env
                            document.close() // Close previous env
                            return;
                        }
                    }
                    // No match so put lastEnv back on a list of valid
    ...
    this.getMathMode = function () {
        ...
        var currentEnv = document.getCurrentEnv();
        if (currentEnv) {
            return currentEnv.mathMode;
        } else {
            return null;
        }
    };
    ...
    var resetMathMode = function () {
        ...
        // TODO: Don't access document.stack directly
        var currentEnv = document.getCurrentEnv();
        if (currentEnv) {
            var lastMathMode = currentEnv.mathMode;
            do {
                var lastEnv = document.close();
