...
var InterpretTokens = function (TokeniseResult, ErrorReporter) {
    ...
    for (var i = 0, len = Tokens.length; i < len; i++) {
        ...
        if (type === "{") {
            ...
            // next group will not be in math mode or undefined unless otherwise specified below
            nextGroupMathMode = null;
        };
        ...
        if (type === "\\") {
            ...
            if (seq === "begin" || seq === "end") {
                ...
                if (newPos === null) { continue; } else {i = newPos;};
            } else if (seq === "hbox" || seq === "text" || seq === "mbox" || seq === "footnote" || seq === "intertext" || seq === "shortintertext" || seq === "textnormal" || seq === "tag" || seq === "reflectbox") {
            ...
            insideGroup = Environments.insideGroup();  // true if inside {....}
            if (currentMathMode === null && !insideGroup) {
...
var EnvHandler = function (ErrorReporter) {
    ...
    this.push = function (newEnv) {
        ...
        //var p = state.length ;
        //var mathModeBefore = state.length > 0 ? !!state[state.length-1].mathMode : null;
        this.checkAndUpdateState(newEnv);
        envs.push(newEnv);
        // Debugging code for math mode transitions
        // var q = state.length;
        // if (true || newEnv.name) {
        //     var mathModeAfter = state.length > 0 ? !!state[state.length-1].mathMode : null;
