...
// Reads the optional param, if present, and returns the position after it.
// If there's no optional param, returns the original position.
// If there's an invalid param, returns null.
var readOptionalParams = function(TokeniseResult, k) {
    ...
    // Check that the argument starts with [, ignore anything else
    if(params && params[1] === "Text") {
        var paramNum = text.substring(params[2], params[3]);
        if (!paramNum.match(/^\[/)) {
            return k; // no optional argument here
        } else {
            /* continue */
        }
    } else {
...
var InterpretTokens = function (TokeniseResult, ErrorReporter) {
    ...
    for (var i = 0, len = Tokens.length; i < len; i++) {
        ...
        if (type === "\\") {
            ...
            if (seq === "documentclass") {
              newPos = readOptionalParams(TokeniseResult, i);
              if (newPos !== null){ i = newPos; } else { TokenError(token, "invalid optional param"); };
            }
            else if (seq === "begin" || seq === "end") {
