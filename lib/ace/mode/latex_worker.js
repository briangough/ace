...
var CheckEnvironments = function (Environments, ErrorReporter) {
	...
	if(thisEnv.command === "begin" || thisEnv.command === "{") {
		...
		ErrorFromTo(lastEnv, thisEnv, "unexpected \\end{" + thisEnv.name + "} inside group {", {suppressIfEditing:true});
		...
		ErrorFromTo(lastEnv, thisEnv, "unexpected \\end{" + thisEnv.name + "} after \\begin{" + lastEnv.name + "}");
