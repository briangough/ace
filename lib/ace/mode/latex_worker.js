    ...
    for (var i = 0, len = Tokens.length; i < len; i++) {
        ...
        if (type === "\\") {
            ...
            if (currentMathMode === null && !insideGroup) {
                TokenErrorFromTo(token, token, type + " must be inside math mode", {suppressIfEditing:false});
            };
        }
    };
    return Environments;
...
var EnvHandler = function (ErrorReporter) {
    ...
    this._end = function(thisEnv) {
        ...
        do {
            ...
            if (lastEnv && lastEnv.potentiallyInvalid) {
                retry = true;
            };
            ...
            if (closedBy(lastEnv, thisEnv)) {
                ...
                // then retry the match for \end
                retry = true;
            } else if (lastEnv.command === "$" || lastEnv.command === "$$") {
                // tried to close environment after single $ or $$
                ErrorFromTo(lastEnv, thisEnv,
                            "unclosed " + lastEnv.command + " found at \\end{" + thisEnv.name + "}",
                            {suppressIfEditing:true, errorAtStart: true});
                // discard the open mathmode by not pushing it back on the stack
                // then retry the match for \end
                retry = true;
                ...
                lastEnv.potentiallyInvalid = true;
                state.push(lastEnv);
            }
        } while (retry === true)
    };
    ...
    this._beginMathMode = function (thisEnv) {
        ...
        if (currentMathMode) {
            ErrorFrom(thisEnv, thisEnv.name + " used inside existing math mode " + currentMathMode);
        };
        ...
        if (thisEnv.command === "{") {
            ...
            if (name) {
                if (name.match(/^(document)/)) {
                    ...
                    if (currentMathMode) {
                        ErrorFrom(thisEnv, thisEnv.name + " used inside math mode " + currentMathMode);
                    };
                    newMathMode = null;
                    ...
                    newMathMode = currentMathMode;
                } else if (name.match(/^(math|displaymath|equation|eqnarray|align)\*?$/)) {
