...
var InterpretTokens = function (TokeniseResult, ErrorReporter) {
    ...
    for (var i = 0, len = Tokens.length; i < len; i++) {
        ...
        if (type === "\\") {
            ...
            if (nextIsDollar && (!currentMathMode || currentMathMode == "$$")) {
                Environments.push({command:"$$", token:token});
                i = i + 1;
            } else {
                Environments.push({command:"$", token:token});
            ...
            console.log("type", type, "mathmode", Environments.getMathMode());
        }
    };
    return Environments;
};
...
var EnvHandler = function (ErrorReporter) {
    ...
    this._end = function(thisEnv) {
        ...
        do {
            ...
            if (lastEnv && lastEnv.name === thisEnv.name) {
                ...
                retry = true;
            } else if (lastEnv.command === "left") {
                // tried to close environment after open \left
                ErrorFromTo(lastEnv, thisEnv,
                            "unclosed \\left found at \\end{" + thisEnv.name + "}",
                            {suppressIfEditing:true, errorAtStart: true});
                // discard the open bracket by not pushing it back on the stack
                // then retry the match for \end
                retry = true
            } else if (lastEnv.command === "begin") {
                ...
                // No match so put lastEnv back on the stack
                state.push(lastEnv);
            }
        } while (retry === true)
    };

    this._endGroup = function (thisEnv) {
        // check if environment or group is closed correctly
        var lastEnv = state.pop();

        if (lastEnv && lastEnv.command === "{") {
            ...
            state.push(lastEnv);
        } else if (lastEnv.command === "right") {
            // tried to close bracket after begin environment
            ErrorFromTo(lastEnv, thisEnv, "unexpected \right after \\begin{" + lastEnv.name +"}");
            state.push(lastEnv);
        }
    };

    this._endBracket = function (thisEnv) {
        // check if environment or group is closed correctly
        var lastEnv = state.pop();

        if (lastEnv && lastEnv.command === "left") {
            // closed group correctly
            return;
        } else if (!lastEnv) {
            // unexpected close, nothing was open!
            if (documentClosed) {
                ErrorFromTo(documentClosed, thisEnv, "\\end{" + documentClosed.name + "} is followed by unexpected \\right",{errorAtStart: true, type: "info"});
            } else {
                ErrorTo(thisEnv, "unexpected \\right }");
            };
        } else if (lastEnv.command === "{") {
            // tried to close brackets after open group
            ErrorFromTo(lastEnv, thisEnv, "unexpected \\right after open group {");
            state.push(lastEnv);
        } else if (lastEnv.command === "begin") {
            // tried to close group after begin environment
            ErrorFromTo(lastEnv, thisEnv, "unexpected \\right after \\begin{" + lastEnv.name +"}");
    ...
    this._mathMode = function (thisEnv) {
        ...
        if (lastEnv && lastEnv.command === thisEnv.command) {
            ...
            if (lastEnv.mathMode) {
                console.log("hit", thisEnv.command, "but already in math mode", lastEnv.mathMode);
            };
            thisEnv.mathMode = thisEnv.command;
            state.push(thisEnv);
        };
    };
    ...
    this.checkAndUpdateState = function (thisEnv) {
        if (inVerbatim) {
            ...
            if (name) {
                if (name.match(/^(document)/)) {
                    // case 1, must be outside math mode
                    if (currentMathMode) { console.log("error", name, "inside", currentMathMode); }
                    newMathMode = null
                } else if (name.match(/^(array)/)) {
                    // case 2, must be inside math mode
                    if (!currentMathMode) { console.log("error", name, "outside", currentMathMode); }
                    newMathMode = currentMathMode;
                } else if (name.match(/^(equation|eqnarray)$/)) {
                    // case 3, must be outside math mode but start it
                    if (currentMathMode) { console.log("error", name, "inside", currentMathMode); }
                    newMathMode = name;
                } else {
                    // case 4, unknown environments
                    newMathMode = undefined;
                }
            }
            ...
            this._endGroup(thisEnv);
        } else if (thisEnv.command === "right") {
            this._endBracket(thisEnv);
        } else if (thisEnv.command === "$" || thisEnv.command === "$$") {
            this._mathMode(thisEnv);
        }
    };
    ...
    this.close = function () {
        ...
        while (state.length > 0) {
            ...
            if (thisEnv.command === "{") {
                ...
                ErrorFrom(thisEnv, "unclosed inline math");
            } else if (thisEnv.command === "$$") {
                ErrorFrom(thisEnv, "unclosed display math");
            } else if (thisEnv.command === "left") {
                ErrorFrom(thisEnv, "unclosed \\left");
            } else if (thisEnv.command === "right") {
                ErrorFrom(thisEnv, "unexpected \\right");
            };
        }
